...
AIME. Among the prompt-level baselines, \textsc{budget\_prompt} barely
restrains generation at all (4{,}514 tokens on MATH-500 against
\textsc{noop}'s 4{,}653), so it is accuracy-preserving but not an efficiency
method; \textsc{concise\_prompt} restrains GSM8K aggressively but not MATH.

\paragraph{A caveat on the trial-decode baseline.} Our
\textsc{trial\_decode} row is far below the near-lossless numbers reported by
DEER \cite{deer} and Dynasor \cite{dynasor}, and we do not present it as a
faithful evaluation of that family. Its exit rule has free parameters
(agreement count, minimum chunks before probing) which we ran at a single
default configuration rather than sweeping, whereas \textsc{exit\_only}
received nine dev configurations and the budget families four each. The
resulting operating point is far more aggressive than any we selected for
ourselves ($317$ vs $2{,}312$ mean think tokens), which is sufficient to
explain the gap. We report the row for completeness and explicitly do not
draw a comparative conclusion from it; a properly tuned head-to-head against
